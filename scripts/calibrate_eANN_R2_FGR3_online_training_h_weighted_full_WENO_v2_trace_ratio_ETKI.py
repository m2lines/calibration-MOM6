...
############## To get started ################
#  python-jl calibrate_eANN_R2_FGR3_online_training_h_weighted_full_WENO_v2_trace_ratio_ETKI.py --echo

## Global paths
TAG = 'TR'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/online_training'
optimization_folder = 'R2_FGR3_h_weighted_full_WENO_v2_trace_ratio_ETKI'
...
for iteration in range(int(metrics['latest_iteration'].item()), N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        If we simply set
        trace(I) = dt * trace(G^T Γ^{-1} G),
        we will have constant contraction rate.
        ...
        dt = Nens_nonan / (np.trace(G.T @ (1. / Gamma[:,np.newaxis] * G)) + 1e-30)
