...
############## To get started ################
#  python-jl calibrate_NW2_5days.py --echo

## Global paths
TAG = 'NW5'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/NW2'
optimization_folder = 'base_5days'
...
commandline = f'cd /home/pp2681/calibration/scripts; sbatch --time=01:00:00 --cpus-per-task=4 --mem=32GB --dependency=singleton --export=NONE --job-name={TAG} -o {base_path}/{optimization_folder}/slurm-%j.out -e {base_path}/{optimization_folder}/slurm-%j.err --wrap="python-jl {script_name}"'
...
if os.path.exists(f'{base_path}/{optimization_folder}/metrics.nc'):
    ...
    for var in metrics.data_vars:
        if var == 'latest_iteration':
            continue
        metrics[var].isel(iter=slice(0, latest_iteration))[:] = \
            metrics_read[var].isel(iter=slice(0, latest_iteration)).copy()
...
for iteration in range(int(metrics['latest_iteration'].item()), N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        for ens_member, param in enumerate(params.T):
            try:
                data = xr.open_mfdataset(f'{iteration_path}/ens-member-{ens_member:02d}/output/snapshots_*.nc', decode_times=False).sortby('time').fillna(0.).isel(zi=slice(0,-1)).isel(time=0).squeeze()
        ...
        for ens_member, param in enumerate(params.T):
            ...
            hpc = HPC.add(name=TAG, time=1, ntasks=1, mem=2, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-dev-m2lines-Aug18')
