...
commandline = f'cd /home/pp2681/calibration/scripts; sbatch --time=01:00:00 --cpus-per-task=4 --mem=64GB --dependency=singleton --export=NONE --job-name={TAG} -o {base_path}/{optimization_folder}/slurm-%j.out -e {base_path}/{optimization_folder}/slurm-%j.err --wrap="python-jl {script_name}"'
...
# Model configuration
exp_params = PARAMETERS_NW2.add(DAYMAX=20.0, MAXTRUNC=0).add(USE_ZB2020='True', ZB_SCALING = 1.0, ZB_KLOWER_R_DISS = 1.0, ZB_KLOWER_SHEAR = 1,  ZB2020_USE_ANN='True', ZB2020_ANN_FILE_TALL='INPUT/Tall.nc', USE_CIRCULATION_IN_HORVISC='True')
# Read necessary NETCDF files
ANN_netcdf_default = xr.open_dataset(f'{ANN_default_path}/eANN.nc').load()
# Read observation vector
observation = xr.open_dataset('/scratch/pp2681/mom6/Neverworld2/simulations/R32/R2_FGR3/EKI_dataset.nc')
...
for iteration in range(int(metrics['latest_iteration'].item()), N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        for ens_member, param in enumerate(params.T):
            try:
                data = xr.open_mfdataset(f'{iteration_path}/ens-member-{ens_member:02d}/output/snapshots_*.nc', decode_times=False).sortby('time').fillna(0.).isel(zi=slice(0,-1))
                g_ens[:,ens_member] = data.e.values.ravel().astype('float64')
            except:
        ...
        for ens_member, param in enumerate(params.T):
            ...
            hpc = HPC.add(name=TAG, time=3, ntasks=1, mem=2, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-dev-m2lines-Aug18')
...
exp_params = exp_params.add(DAYMAX=2000., MAXTRUNC=100000, ZB2020_ANN_FILE_TALL='../iteration-09/ens-member-00/INPUT/Tall.nc')
...
experiment_folder = f'{base_path}/{optimization_folder}/final_experiment_09'
further_command = f'cp /scratch/pp2681/mom6/Neverworld2/simulations/R32/R2_FGR3/RESTART/MOM_0.res.nc {experiment_folder}/INPUT/MOM.res.nc'
run_experiment(experiment_folder, hpc, exp_params, 
