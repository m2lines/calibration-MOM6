...
############## To get started ################
#  python-jl calibrate_eANN_R2_FGR3_series_obs_best.py --echo

## Global paths
TAG = 'ser'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/20-years'
optimization_folder = 'R2_FGR3_series_obs_best'
...
for iteration in range(first_iteration, N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        # Find best element
        residuals = g_ens - y[:, None]
        dist2 = np.sum((residuals**2) / Gamma[:, None], axis=0)
        j_star = np.nanargmin(dist2)
        print("Nearest ensemble member:", j_star)

        # See prediction of new ensemble by ETKI
        params_new = Main.eval("get_u_final(eki)")
        # Replace mean
        params_new_mean = params_new.mean(-1, keepdims=True)
        params_new_residual = params_new - params_new_mean

        params_new = params[:,j_star:j_star+1] + params_new_residual

        iteration_path = f'{base_path}/{optimization_folder}/iteration-{iteration+1:02d}'
        params_file = f'{iteration_path}-params.txt'
        ...
        for ens_member, param in enumerate(params.T):
            for RR in ['R2']:
                ...
                '/home/pp2681/MOM6-examples/build/configurations/double_gyre', call_function, further_command)
