...
############## To get started ################
#  python-jl calibrate_eANN_R2_FGR3_series_scale.py --echo

## Global paths
TAG = 'scale'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/20-years'
optimization_folder = 'R2_FGR3_series_scale'
...
for iteration in range(N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        # # Manual learning rate schedulers which limits the step sizes
        # # We first compute the model error for the ensemble mean
        # model_error = (y - np.nanmean(g_ens, axis=1))
        # # Second, we scale model error with the weighting matrix
        # model_error = model_error / Gamma
        # # Third, we compute a vector of scalar products
        # scalar_products = g_ens.T @ model_error
        # # Fourth, we compute second norm while skipping nan entries
        # obs_cov_inflation_factor = np.sqrt(np.nansum(scalar_products**2))
        # # We multiply the observational covariance matrix by the
        # # obs_cov_inflation_factor. To do so, we introduce the time
        # # step in EKI which is inverse of this number

        # print('Observational covariance inflation factor by my method', obs_cov_inflation_factor, 'time step', 1. / obs_cov_inflation_factor)
        # Main.dt = 1. / obs_cov_inflation_factor
