...
############## To get started ################
#  python-jl calibrate_eANN_WENO_R2_FGR3_series_01.py --echo

## Global paths
TAG = 'WNO1'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/20-years'
optimization_folder = 'WENO_R2_FGR3_series_01'
...
# Model configuration
exp_params = PARAMETERS.add(DAYMAX=7300.0).add(USE_ZB2020='True',ZB2020_USE_ANN='True',ZB2020_ANN_FILE_TALL='INPUT/Tall.nc',USE_CIRCULATION_IN_HORVISC='True').add(
    ...
    if os.path.exists(iteration_path):
        ...
        for ens_member, param in enumerate(params.T):
            for RR in ['R2']:
                ...
                if RR == 'R2':
                    hpc = HPC.add(name=TAG, time=2, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-WENO-ANN-boundary')
                if RR == 'R4':
