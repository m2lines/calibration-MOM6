...
############## To get started ################
#  python-jl calibrate_eANN_R4_FGR3.py --echo

## Global paths
TAG = 'R4'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/20-years'
optimization_folder = 'R4_FGR3'
...
observation = xr.open_dataset('/home/pp2681/calibration/scripts/R32/R4_FGR3.nc')
...
# 7040*2 values of interface mean
y1 = (observation.e_mean).values.ravel().astype('float64')
# 7040*2 values of interface std
...
for iteration in range(N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        for ens_member, param in enumerate(params.T):
            try:
                y_prediction = []
                for res_idx, coarse_factor, RR in zip([0], [1], ['R4']):
                    e = xr.open_mfdataset(f'{iteration_path}/{RR}/ens-member-{ens_member:02d}/output/prog_*.nc', decode_times=False).e.sel(Time=slice(365*10,None)).isel(zi=slice(0,2))
                    e_mean = e.mean('Time')
                    e_std = e.std('Time')
        ...
        for ens_member, param in enumerate(params.T):
            for RR in ['R4']:
                ...
                if RR == 'R4':
                    hpc = HPC.add(name=TAG, time=3, ntasks=4, mem=2, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-dev-m2lines-Aug18')
                if RR == 'R8':
