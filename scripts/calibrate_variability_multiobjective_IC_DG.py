...
############## To get started ################
#  python-jl calibrate_variability_multiobjective_IC_DG.py --echo
...
optimization_folder = 'EKI-Vanilla-IC-e-mean-std-spread-0.1'
...
initial_metrics = xr.open_dataset('/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/variability-R2/EKI-Vanilla-e-mean-std-spread-0.1/metrics.nc')
initial_parameters = initial_metrics['param'].isel(iter=3).mean(dim='ens').values
initial_ensemble = initial_parameters.reshape(-1,1) + np.concatenate([A1_std * np.random.randn(len(A1_mean), N_ensemble), b1_std * np.random.randn(len(b1_mean), N_ensemble)]).astype('float64')
