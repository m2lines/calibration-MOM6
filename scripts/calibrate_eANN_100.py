...
############## To get started ################
#  python-jl calibrate_eANN_100.py --echo
...
# Observational noise for three resolutions
Gamma = np.concatenate([Gamma, Gamma, Gamma])
...
ny = 40
nx = 44
metrics['e_mean'] = xr.DataArray(np.nan * np.zeros([N_iterations, N_ensemble, Nres, ny, nx]), dims=['iter', 'ens', 'res', 'yh', 'xh'])
metrics['e_std'] = xr.DataArray(np.nan * np.zeros([N_iterations, N_ensemble, Nres, ny, nx]), dims=['iter', 'ens', 'res', 'yh', 'xh'])
metrics['param'] = xr.DataArray(np.nan * np.zeros([N_iterations, N_ensemble, initial_ensemble.shape[0]]), dims=['iter', 'ens', 'pdim'])
metrics['RMSE_e_mean'] = xr.DataArray(np.nan * np.zeros([N_iterations, N_ensemble, Nres]), dims=['iter', 'ens', 'res'])
metrics['RMSE_e_std'] = xr.DataArray(np.nan * np.zeros([N_iterations, N_ensemble, Nres]), dims=['iter', 'ens', 'res'])
...
for iteration in range(N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        g_ens = np.zeros([40*44*2*3, N_ensemble]).astype('float64')
        for ens_member, param in enumerate(params.T):
            try:
                y_prediction = []
                for res_idx, coarse_factor, RR in zip([0,1,2], [1,2,4], ['R2', 'R4', 'R8']):
                    ...
                    data = ds.e.sel(Time=slice(365*10,None)).isel(zi=0).coarsen({'xh':coarse_factor, 'yh':coarse_factor}).mean().compute()
                    e_mean = data.mean('Time').compute()
                    e_std = data.std('Time').compute()

                    y_prediction.append(e_mean.values.ravel().astype('float64'))
                    y_prediction.append(e_std.values.ravel().astype('float64'))
