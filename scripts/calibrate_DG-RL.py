...
optimization_idx = 'RL-EKI-ens-size-20'
N_iterations = 5
...
# One realization of the observation noise
np.random.seed(0)
noise = np.sqrt(0.01) * np.random.randn(N_ensemble, ny*nx)
metrics['noise'] = xr.DataArray(noise.reshape([N_ensemble, ny, nx]), dims=['ens', 'yh', 'xh'])
...
for iteration in range(N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        for ens_member, param in enumerate(params.T):
            try:
                ...
                if len(error) != 440:
                    ...
                    sys.exit(1)   # terminate immediately with error code
                g_ens[:,ens_member] = error + noise[ens_member]
        ...
        for ens_member, param in enumerate(params.T):
            ...
            hpc = HPC.add(name='RL', mem=2, ntasks=4, time=2, executable='/home/pp2681/MOM6-examples/build/compiled_executables/MOM6-ZB-2023')
        ...
        os.system(f'cd /home/pp2681/calibration/scripts; sbatch --mem=16GB --dependency=singleton --export=NONE --job-name=RL -o {base_path}/{optimization_idx}/slurm-%j.out --wrap="python-jl calibrate_DG-RL.py"')
