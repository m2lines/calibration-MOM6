...
############## To get started ################
#  python-jl calibrate_eANN_R2_FGR3_online_training_h_weighted_full_WENO_v2_convex_ETKI.py --echo

## Global paths
TAG = 'con'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/online_training'
optimization_folder = 'R2_FGR3_h_weighted_full_WENO_v2_convex_ETKI_std_alpha'
...
# Model configuration
exp_params = PARAMETERS.add(DAYMAX=20.0).add(USE_ZB2020='True',ZB2020_USE_ANN='True',ZB2020_ANN_FILE_TALL='INPUT/Tall.nc',USE_CIRCULATION_IN_HORVISC='True').add(
...
# 89 x 100 matrix; (For 64 neurons and N8 symmetries)
...
for iteration in range(int(metrics['latest_iteration'].item()), N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        Main.g_ens = g_ens
        # Compute as large step as possible for ETKI under an assumption of small step,
        # such that the ensemble mean is in the convex hull of the updated ensemble,
        # i.e., the weights in the affine combination are non-negative.
        # We remind that g_ens.shape = Nobs, Nens

        g_mean = np.nanmean(g_ens, axis=1)
        alpha = (g_ens - g_mean.reshape(-1,1)).T @ (1. / Gamma * (y - g_mean))
        print('Alpha shape', alpha.shape, 'Alpha min/mean/max/std', np.nanmin(alpha), np.nanmean(alpha), np.nanmax(alpha), np.nanstd(alpha))
        dt = 1 / (np.nanstd(alpha) + 1e-30)
        Nens_nonan = np.sum(~np.isnan(g_ens[0,:]))
        w = 1/Nens_nonan + dt / (Nens_nonan - 1) * (alpha - np.nanmean(alpha))
        print('Computed ETKI step size dt = ', dt)
        print('Range of alpha', alpha)
        print('Expected weights of convex hull', w)
        print('Sum of weights', np.nansum(w))
        print('Effective sample size', 1. / np.nansum(w**2))
        Main.dt = dt
        ...
        for ens_member, param in enumerate(params.T):
            for RR in ['R2']:
                ...
                if RR == 'R2':
                    hpc = HPC.add(name=TAG, time=1, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-WENO-ANN-boundary')
                if RR == 'R4':
                    hpc = HPC.add(name=TAG, time=12, ntasks=4, mem=2, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-WENO-ANN-boundary')
                if RR == 'R8':
                    hpc = HPC.add(name=TAG, time=48, ntasks=16, mem=4, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-WENO-ANN-boundary')
...
hpc = HPC.add(name=TAG, time=1, ntasks=4, mem=2, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-WENO-ANN-boundary')
...
run_experiment(experiment_folder, hpc, exp_params.add(**configuration('R2')), '', further_command)