...
############## To get started ################
#  python-jl calibrate_eANN_R2_FGR3_online_training_inflation.py --echo

## Global paths
TAG = 'INF'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/online_training'
optimization_folder = 'R2_FGR3_inflation'
...
for iteration in range(N_iterations):
    ...
    if os.path.exists(iteration_path):
        ...
        Main.g_ens = g_ens
        # Here we use simple s=1 because given dt=0.5, we will increate the covariance twice 
        # Compare to the algorithm without inflation
        # Additionally, we use time step 0.5, which corresponds to firtst two steps of DMC.
        # Further steps of DMC increase the step (and we prevent this by keeping the time step to be always 0.5)
        # We do not use DMC because it can unexpectedly increase or decrease the step size
        # and this will considerably complicate choosing the inflation factor.
        ...
        Main.eval("update_ensemble!(eki, g_ens; multiplicative_inflation = true, s = 1.0)")
