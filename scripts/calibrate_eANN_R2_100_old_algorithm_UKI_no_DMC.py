...
############## To get started ################
#  python-jl calibrate_eANN_R2_100_old_algorithm_UKI_no_DMC.py --echo

## Global paths
TAG = 'ukin'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/hundred-years'
optimization_folder = 'R2-old-algorithm-UKI-no-DMC'
...
ENS_SPREAD = 0.25
sigma1 = ENS_SPREAD * float(ANN_netcdf_default['weights2'].values.std())
sigma2 = ENS_SPREAD * float(np.abs(ANN_netcdf_default['biases2'].values).mean())
mu1 = ANN_netcdf_default['weights2'].values
mu2 = ANN_netcdf_default['biases2'].values
Main.eval(f"""
          using EnsembleKalmanProcesses.ParameterDistributions
          prior_0 = constrained_gaussian("p0", {mu1[0]}, {sigma1}, -Inf, Inf)
          prior_1 = constrained_gaussian("p1", {mu1[1]}, {sigma1}, -Inf, Inf)
          prior_2 = constrained_gaussian("p2", {mu1[2]}, {sigma1}, -Inf, Inf)
          prior_3 = constrained_gaussian("p3", {mu1[3]}, {sigma1}, -Inf, Inf)
          prior_4 = constrained_gaussian("p4", {mu1[4]}, {sigma1}, -Inf, Inf)
          prior_5 = constrained_gaussian("p5", {mu1[5]}, {sigma1}, -Inf, Inf)
          prior_6 = constrained_gaussian("p6", {mu1[6]}, {sigma1}, -Inf, Inf)
          prior_7 = constrained_gaussian("p7", {mu1[7]}, {sigma1}, -Inf, Inf)
          prior_8 = constrained_gaussian("p8", {mu1[8]}, {sigma1}, -Inf, Inf)
          prior_9 = constrained_gaussian("p9", {mu1[9]}, {sigma1}, -Inf, Inf)
          prior_10 = constrained_gaussian("p10", {mu1[10]}, {sigma1}, -Inf, Inf)
          prior_11 = constrained_gaussian("p11", {mu1[11]}, {sigma1}, -Inf, Inf)
          prior_12 = constrained_gaussian("p12", {mu2[0]}, {sigma2}, -Inf, Inf)
          prior = combine_distributions([prior_0, prior_1, prior_2, prior_3, prior_4, prior_5, prior_6, prior_7, prior_8, prior_9, prior_10, prior_11, prior_12])
          """)
...
Main.eval("""
    ...
    y, Diagonal(Γ), Unscented(prior, impose_prior=true),
...
metrics['param'] = xr.DataArray(np.nan * np.zeros([N_iterations, N_ensemble, n_param]), dims=['iter', 'ens', 'pdim'])
    ...
    if os.path.exists(iteration_path):
        ...
        for ens_member, param in enumerate(params.T):
            for RR in ['R2']:
                ...
                if RR == 'R2':
                    hpc = HPC.add(name=TAG, time=3, ntasks=2, begin='1minute', executable='/scratch/pp2681/MOM6-examples/build/compiled_executables/MOM6-dev-m2lines-Aug18')
                if RR == 'R4':
