...
############## To get started ################
#  python-jl calibrate_eANN_R2_FGR3_series_100.py --echo

## Global paths
TAG = 'ser'
base_path = '/scratch/pp2681/mom6/CM26_Double_Gyre/calibration/hundred-years'
optimization_folder = 'R2_FGR3_series'
...
commandline = f'cd /home/pp2681/calibration/scripts; sbatch --time=06:00:00 --mem=16GB --dependency=singleton --export=NONE --job-name={TAG} -o {base_path}/{optimization_folder}/slurm-%j.out -e {base_path}/{optimization_folder}/slurm-%j.err --wrap="python-jl {script_name}"'
        ...
        for ens_member, param in enumerate(params.T):
            ...
            for res_idx, coarse_factor, RR in zip([0], [1], ['R2']):
                try:
                    ...
                    e_mean = e.mean('Time').values
                    e_std = e.std('Time').values
                ...
                y_prediction.append(e_mean.ravel())
                y_prediction.append(e_std.ravel())
