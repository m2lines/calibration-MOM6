...
commandline = f'cd /home/pp2681/calibration/scripts_torch; sbatch --time=00:30:00 --cpus-per-task=4 --mem=16GB --dependency=singleton --export=NONE --job-name={TAG} -o {base_path}/{optimization_folder}/slurm-%j.out -e {base_path}/{optimization_folder}/slurm-%j.err --wrap="python-jl {script_name}"'
...
observation = xr.open_dataset('/home/pp2681/calibration/scripts/R32/R2_FGR3_online_training.nc').isel(Time=slice(0,20))
        ...
        for ens_member, param in enumerate(params.T):
            try:
                ...
                for res_idx, coarse_factor, RR in zip([0], [1], ['R2']):
                    data = xr.open_mfdataset(f'{iteration_path}/{RR}/ens-member-{ens_member:02d}/output/prog_*.nc', decode_times=False).sortby('Time').fillna(0.).drop_vars('Time')
